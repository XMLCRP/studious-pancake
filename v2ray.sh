...
~/.acme.sh/acme.sh --installcert -d "$domainName" --fullchain-file $ssl_dir/v2ray.crt --key-file $ssl_dir/v2ray.key
...
[ "vless" = "$3" ] && sed -i 's/vmess/vless/' $v2rayConfig
...
echo "端口: 443"
echo "UUID: $uuid"
[ "vless" = "$3" ] && echo "协议：vless" || echo "额外ID: 0"
echo "安全: tls"
echo "传输: websocket"
echo "路径: $path"